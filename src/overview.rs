...
enum Nameless {
    ...
    /// A `pyproject.toml` with `[build-system]` but no `[project]`: the ordinary Python layout
    /// with metadata in `setup.cfg`.
    BuildSystemOnly,
...
impl Nameless {
    ...
    fn label(self) -> String {
        match self {
            ...
            // A bare `[workspace]` with no `members` key: still worth saying it is a workspace,
            // but there is no count to report.
...
fn parse_cargo_toml(root: &Path) -> Result<ManifestInfo, String> {
    ...
    let nameless = match &parsed.workspace {
        Some(ws) => Nameless::CargoWorkspace(ws.members.as_ref().map_or(0, Vec::len)),
...
fn parse_pyproject_toml(root: &Path) -> Result<ManifestInfo, String> {
    ...
    struct PyProject {
        ...
        build_system: Option<toml::Table>,
    }
    #[derive(Default, Deserialize)]
    struct Project {
        name: Option<String>,
    ...
    // Same precedence as Cargo's: a `[build-system]` table means the file is not empty.
    let nameless = if parsed.build_system.is_some() {
